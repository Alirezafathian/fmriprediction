...
def general(ds,ct,negative_corr,tm,tv,enodes_name,selected_set,nb,binM):
...
#===============================================================
    ...
    #if enodes_name!='all':
    #    Ntopdir = glob.glob(featuresDIR + '/*nodes-%s*%s*'%(enodes_name,sgn))
    #    Ltopdir = glob.glob(featuresDIR + '/*links-%s*%s*'%(enodes_name,sgn))
...
#===============================================================
    ...
    for i in strlist:
        ...
        for sub in directoies[str(i)]:
            iid = str(sub.split("/sub-")[1].split('/sub-')[0])
            if iid not in all_sub_list:
                continue
            measures['all'][str(i)][iid] = pd.read_csv(sub, sep='\t')
            measures['all'][str(i)][iid] = measures['all'][str(i)][iid].drop(['Unnamed: 0'], axis=1)
            if 'loc' in str(i):
                measures['all'][str(i)][iid] = measures['all'][str(i)][iid].set_index('ID')
                if enodes_name!='all':
                    measures['all'][str(i)][iid] = measures['all'][str(i)][iid].loc[enodes]
            for g in subjects_groups:
                if iid in subjects_list[g]:
                    measures[g][str(i)][iid] = pd.read_csv(sub, sep='\t')
                    measures[g][str(i)][iid] = measures[g][str(i)][iid].drop(['Unnamed: 0'], axis=1)
                    if 'loc' in str(i):
                        measures[g][str(i)][iid] = measures[g][str(i)][iid].set_index('ID')
                        if enodes_name!='all':
                            measures[g][str(i)][iid] = measures[g][str(i)][iid].loc[enodes]
#===============================================================
    directoies = {}
    for ds in denoising_strategies:
        dirc = '%s/data/06_network_measures/positive/tm-%s/corr-%s/ds-%s/'%(rootdir,tm,ct,ds)
...
#===============================================================
    ...
    for i in strlist:
        ...
        for sub in directoies[str(i)]:
            ...
            if iid not in all_sub_list:
                continue
            RAWmeasures['all'][str(i)][iid]    = pd.read_csv(sub, sep='\t')
            RAWmeasures['all'][str(i)][iid]    = RAWmeasures['all'][str(i)][iid].drop(['Unnamed: 0'], axis=1)
            if 'loc' in str(i):
                RAWmeasures['all'][str(i)][iid] = RAWmeasures['all'][str(i)][iid].set_index('ID')
                if enodes_name!='all':
                    RAWmeasures['all'][str(i)][iid] = RAWmeasures['all'][str(i)][iid].loc[enodes]
            for g in subjects_groups:
                if iid in subjects_list[g]:
                    RAWmeasures[g][str(i)][iid] = pd.read_csv(sub, sep='\t')
                    RAWmeasures[g][str(i)][iid] = RAWmeasures[g][str(i)][iid].drop(['Unnamed: 0'], axis=1)
                    if 'loc' in str(i):
                        RAWmeasures[g][str(i)][iid] = RAWmeasures[g][str(i)][iid].set_index('ID')
                        if enodes_name!='all':
                            RAWmeasures[g][str(i)][iid] = RAWmeasures[g][str(i)][iid].loc[enodes]
...
#===============================================================
    ...
    ##    for i in removed_subL:
    ##        for j in removed_subL:
    ##            meanM_adj[subjects_groups[g]][i][j] = 0.0
    ##    np.fill_diagonal(meanM_adj[subjects_groups[g]], 0.0)
    ...
    ##for i in removed_subL:
    ##    for j in removed_subL:
    ##        meanM_adj['all'][i][j] = 0.0
    ##np.fill_diagonal(meanM_adj['all'], 0.0)
    ...
    return group_couples,sep,all_sub_list,all_subjects,subjects_list,sg,str_loc,str_glob,strlist,tm,sgn,measures,RAWmeasures,intersect,removed,removed_subL,r,cols,subL,subLRAW,subL_adj,subLRAW_adj,meanM,meanG,meanM_bin,meanG_bin,meanM_adj,meanG_adj
#============================================================================================================
